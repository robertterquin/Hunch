...
function ReportListCard({ report, selected, onSelect }: { report: AnalysisReport; selected: boolean; onSelect: () => void }) {
  return <article className={`saved-report-card${selected ? ' is-selected' : ''}`}><button className={`select-report${selected ? ' is-selected' : ''}`} type="button" onClick={onSelect} aria-label={`${selected ? 'Deselect' : 'Select'} ${getFixtureTitle(report.fixtureId)}`}><Check size={15} aria-hidden="true" /></button><Link to={`/saved/${report.id}`} className="saved-report-main"><div className="saved-report-heading"><strong>{getFixtureTitle(report.fixtureId)}</strong><span className={`risk-pill risk-pill-${report.riskLevel}`}>{riskLabel(report)}</span></div><p>{report.summary}</p><div className="saved-report-meta"><span>{report.riskScore}/100</span><span>{report.flags.length} signals</span><span>{formatDate(report.createdAt)}</span></div></Link><Link className="text-link" to={`/saved/${report.id}`} aria-label={`Open ${getFixtureTitle(report.fixtureId)}`}><ArrowRight size={17} aria-hidden="true" /></Link></article>
}
...
export function SavedPage() {
  ...
  const visibleReports = useMemo(() => savedReports.filter((report) => {
    const matchesQuery = getFixtureTitle(report.fixtureId).toLowerCase().includes(query.toLowerCase()) || report.originalText.toLowerCase().includes(query.toLowerCase())
...
export function SettingsPage() {
  const { savedReports, deleteAllReports } = useAppState()
  const [privacy, setPrivacy] = useState(true)
  const [message, setMessage] = useState('')
  const clearReports = () => { if (window.confirm('Delete all saved reports from this session?')) { deleteAllReports(); setMessage('All saved reports were removed from this session.') } }
  return <SupportPage eyebrow="Settings" title="Account and privacy" description="Control what Hunch keeps in this browser session and how you return to your saved checks."><section className="settings-stack"><div className="panel settings-row"><div className="settings-icon"><Settings size={19} aria-hidden="true" /></div><div><h2>Anonymous session</h2><p className="muted-copy">You can analyze without an account. Saving reports will require authentication when Supabase is connected.</p></div><Link className="button button-secondary" to="/auth/sign-in">Sign in</Link></div><div className="panel settings-row"><div className="settings-icon"><LockKeyhole size={19} aria-hidden="true" /></div><div><h2>Privacy notice</h2><p className="muted-copy">Hunch should keep submitted text in the active flow unless you explicitly save a report.</p></div><button className={`toggle${privacy ? ' is-on' : ''}`} type="button" onClick={() => setPrivacy((current) => !current)} aria-pressed={privacy}><span />{privacy ? 'On' : 'Off'}</button></div><div className="panel settings-row settings-danger"><div className="settings-icon"><Trash2 size={19} aria-hidden="true" /></div><div><h2>Delete saved data</h2><p className="muted-copy">{savedReports.length} saved report{savedReports.length === 1 ? '' : 's'} in the current demo session.</p></div><button className="button button-secondary" type="button" onClick={clearReports} disabled={savedReports.length === 0}>Delete all</button></div>{message && <div className="notice notice-success" role="status"><Check size={16} aria-hidden="true" />{message}</div>}</section></SupportPage>
}

export function AuthPage() {
  const { mode = 'sign-in' } = useParams()
  const { activeReport, saveReport } = useAppState()
  const [email, setEmail] = useState('')
  const [error, setError] = useState('')
  const [isAuthenticated, setIsAuthenticated] = useState(false)
  const title = mode === 'sign-up' ? 'Create an account' : mode === 'reset' ? 'Reset your password' : 'Sign in to save reports'
  const submit = () => { if (!email.includes('@')) { setError('Enter a valid email address to continue.'); return } setError(''); if (activeReport) saveReport(activeReport); setIsAuthenticated(true) }
  return <SupportPage eyebrow="Account" title={title} description={activeReport ? 'Your current report will stay available while you sign in. After authentication, it can be saved privately.' : 'Create a private account to save and revisit your reports.'}><section className="panel auth-panel"><div className="auth-heading"><div className="settings-icon"><LockKeyhole size={19} aria-hidden="true" /></div><div><strong>{activeReport ? 'Your report is preserved' : 'Private reports, when you are ready'}</strong><p className="muted-copy">No report is saved until you choose to continue.</p></div></div>{isAuthenticated ? <div className="auth-success"><Check size={22} aria-hidden="true" /><h2>Signed in for this demo session.</h2><p>Your report has been preserved in Saved.</p><Link className="button button-primary" to={activeReport ? "/analyze" : "/saved"}>{activeReport ? 'Return to Analyze' : 'Open Saved'} <ArrowRight size={16} aria-hidden="true" /></Link></div> : <div className="auth-form"><label className="field-label" htmlFor="email">Email address</label><input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email" />{error && <p className="inline-error" role="alert"><CircleAlert size={16} aria-hidden="true" />{error}</p>}<button className="button button-primary" type="button" onClick={submit}>Continue <ArrowRight size={16} aria-hidden="true" /></button><p className="muted-copy">This is a Phase 7 demo state. Supabase session handling arrives in Phase 10.</p></div>}<Link className="text-link" to="/analyze"><ArrowLeft size={14} aria-hidden="true" />Return to Analyze</Link></section></SupportPage>
}
